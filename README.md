...
| 기능 | 설명 |
|------|------|
| **할 일 & 체크리스트** | 할 일 CRUD, 우선순위, 메모 기능 및 **항목 내 하위 체크리스트** 지원 |
| **카테고리 탭** | 여러 탭 생성·이름 변경·삭제, 탭별 독립 목록 관리 |
| **세밀한 자동 초기화** | 전역 또는 항목별(매일/매주/매월/매년/N일마다/N주마다) 시작일을 기준 삼아 완료 체크 자동 해제 |
| **프리셋 (Preset) 공유** | 본인의 탭과 할 일 목록을 6자리 코드로 생성하여 다른 사용자와 공유(가져오기) 가능 |
| **UI 테마 & 팔레트** | 라이트/다크 테마 지원, 10가지 프리셋 및 **Hex 코드 커스텀 팔레트 (글자색 자동 반전)** 기능 |
| **배경 이미지 & UI 숨김** | 고화질 배경 적용(글래스모피즘 효과), 눈 아이콘 클릭 시 UI가 숨겨지며 배경 사진이 부드럽게 확장됨 |
| **Firebase 실시간 동기화** | Google 로그인 연동, PC ↔ Android 간 할 일·카테고리 실시간 자동 동기화 |
| **시스템 알림** | 운영체제 네이티브 알림 통합 지원 (Windows/Linux/Android) |
| **UX 편의 기능** | 카테고리 삭제 시 4초 내 되돌리기(Undo), 미완료/완료 자동 정렬, **드래그 앤 드롭 순서 변경** |
| **데스크톱 편의성** | Always on Top (최상위 창 고정), OS 로그인 시 자동 시작 지원 (Electron 전용) |
...
│       ├── perf.js       # 렌더 횟수 계측 유틸
│       ├── presets.js    # 탭 및 할 일 공유(프리셋) 로직
│       ├── recurrence.js # N일/N주, 매월/매년 등 복합 주기 계산 순수 함수
...
2. **함수형 스케줄러 구현:** `reset.js` 모듈에 전략 패턴(Strategy Pattern)을 도입하여, 타이머 및 자동 초기화(N일/N주 주기 등) 로직의 유지보수성과 확장성을 극대화했습니다.
...
5. **접근성 및 동적 렌더링:** YIQ 색상 대비 알고리즘을 도입하여 사용자가 지정한 팔레트 색상에 따라 글자색이 자동으로 대비(명도)를 맞추도록 구현하였으며, Flexbox 기반 반응형 숨김 구조로 매끄러운 렌더링을 제공합니다.
...
| 데이터 | 저장 위치 | 비고 |
|--------|-----------|------|
| 할 일 목록 | `localStorage` + Firestore (`users/{uid}/todos`) | 실시간 양방향 동기화, 하위 체크리스트 포함 |
| 카테고리 | `localStorage` + Firestore (`users/{uid}/categories`) | 실시간 양방향 동기화 |
| 프리셋 (공유) | Firestore (`presets/{code}`) | **사용자 간 공유**를 위한 루트 컬렉션 (인증 필요) |
| 설정 (테마, 팔레트 등) | `localStorage` + Firestore (`users/{uid}/settings`) | 기기 간 동기화 |
